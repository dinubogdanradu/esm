/**
 * React-PDF (@react-pdf/renderer) template recreating the "Bogdan Dinu"
...
const profileBullets = [
  ...
  'Worked for clients like IQOS (Philip Morris), Vodafone, Ursus, Timisoreana, Joel Robuchon, Alka, BCR, HP, Napolact, Unicredit Bank, NelsonHall, Unilever, Edenia Foods, Tarom, BRD, Janssen and Sodexo',
];

const qualBullets = [
  'Bachelor\u2019s Degree (Electrical Engineering and Computer Science), Bucharest',
  'Masters in Anthropology (not graduated) \u2013 Faculty of Political Sciences (SNSPA), Bucharest',
];

const unGenevaBullets = [
...
const personalProjects = [
  { title: 'UN Geneva Design System', text: 'Starting from the new theme for the ungeneva.org website I created a starterkit theme that was later used on all ITBAU Drupal projects and also projects from other departments. The main objective was to have a unified, easy to use and accessible (WCAG compliant) theme.' },
...
function Header() {
  return (
    <>
      ...
      <View style={styles.namePill}><Text style={styles.nameText}>Bogdan Dinu</Text></View>
      <Text style={styles.titleText}>Fullstack Developer</Text>
      <Text style={[styles.contactText, { left: 572 }]}>bogdanradu.dinu@infosys.com</Text>
      <Text style={[styles.contactText, { left: 797 }]}>Bucharest, Romania</Text>
  ...
  return (
    <Page size={[960, 600]} style={styles.page}>
      ...
      <View style={{ position: 'absolute', top: 181, left: 415, width: 516 }}>
        <Text style={styles.jobTitle}>Senior Drupal Developer &ndash; United Nations Office at Geneva (ITBAU)</Text>
        {unGenevaBullets.map((b, i) => <Bullet key={i}>{b}</Bullet>)}
        <Text style={styles.jobTitle}>Senior Fullstack Web Developer &ndash; Kubis Interactive, Bucharest, Romania</Text>
        {kubisBullets.map((b, i) => <Bullet key={i}>{b}</Bullet>)}
      </View>
    </Page>
  );
}
  ...
  return (
    <Page size={[960, 600]} style={styles.page}>
      ...
      <View style={{ position: 'absolute', top: 75, left: 27, width: 373 }}>
        ...
        <Bullet>Certified Drupal 11 Developer</Bullet>
        <Bullet>Certified Drupal 9 Site Builder</Bullet>
